...
import { startOfWeek, addDays, format, startOfDay, parseISO } from 'date-fns';
...
export const useTrainingStore = create<TrainingState>((set, get) => ({
  ...
  generateSessions: async (startDate, endDate, sessionsPerWeek) => {
    ...
    try {
      ...
      // Normalizziamo le date per lavorare solo con i giorni, ignorando l'orario
      ...
      // Giorni della settimana per la distribuzione
      ...
      while (currentWeekIter <= end) {
        for (const offset of offsets) {
          ...
          newSessions.push({
            userId: user.id,
            seasonId,
            date: format(sessionDate, "yyyy-MM-dd"), // Salviamo in formato stringa semplice
            index: sessionCount,
            notes: ""
          });
        }

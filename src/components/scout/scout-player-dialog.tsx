    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] md:max-w-md rounded-3xl p-0 overflow-hidden border-none shadow-2xl">
        ...
        <ScrollArea className="max-h-[70vh]">
          <div className="p-6 space-y-4">
            <div className="space-y-1.5">
              <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Nome Giocatore</Label>
              <Input 
                value={formData.name} 
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Ruolo</Label>
              ...
              <div className="space-y-1.5">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Squadra Attuale</Label>
                <Input 
            ...
            <div className="space-y-1.5">
              <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Etichette (Seleziona)</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Note Tecniche</Label>
              <Textarea 
                value={formData.notes} 
                onChange={e => setFormData({...formData, notes: e.target.value})}
                placeholder="Ottimo piede sinistro, veloce nel breve..."
                className="min-h-[100px] rounded-xl text-xs font-medium"
              />
            </div>
          </div>
        </ScrollArea>
        ...
        <DialogFooter className="p-6 pt-2 flex-row gap-2">
          <Button variant="ghost" className="flex-1 rounded-xl font-black uppercase text-xs h-12" onClick={() => onOpenChange(false)}>
            Annulla
          </Button>
          <Button 
            className="flex-1 rounded-xl bg-primary text-white font-black uppercase text-xs h-12 shadow-lg"
            onClick={handleSave}
            disabled={loading || !formData.name}
          >
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Salva Talento"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
